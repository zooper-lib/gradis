...
The system SHALL provide a `Railway<E, C>` class that uses an immutable builder pattern for composing guards and steps.

#### Scenario: Creating an empty railway
- **WHEN** a new `Railway<E, C>()` is instantiated
...
- **WHEN** defining a railway with type parameters `Railway<E, C>`
